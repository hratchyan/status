...
`;
  ...
  console.log(`✅ Generated src/lib/api.ts with ${services.services.length} services and ${locations.locations.length} locations`);
}
...
function main() {
  ...
  if (!services.services || !Array.isArray(services.services)) {
    console.error('❌ Error: services.yml must contain a "services" array');
    process.exit(1);
  }
  
  if (!locations.locations || !Array.isArray(locations.locations)) {
    console.error('❌ Error: locations.yml must contain a "locations" array');
    process.exit(1);
  }
